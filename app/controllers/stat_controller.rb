  ...
  def graph_code_gen()
     if params[:survey_id]
        ...
        @survey_sheet.responses.each do | resp |
          ...
          if resp.question.id == params[:question_id]   #TODO: refactor
            logger.debug "//////// find target response of question #{params[:question_id]}  |||||  #{@survey_sheet.responses.response_versions.size}"
            resp.response_versions.each do | ver |
              logger.debug "~~~~~~~~~~~~~~~  for version #{ver.version} ==== rating : #{ver.rating} , answer_text: #{ver.updated_at} "
